...
def getZeta(vecSpikeTimes, matEventTimes, dblUseMaxDur=None, intResampNum=100, intPlot=0,
    ...
    if (len(vecRealDiff) < 3) | (vecSpikeTimes.shape[0] <= 1):
        dblZetaP = 1
        dblZETA = 0
        sZETA = []
        vecLatencies = np.nan
        ...
        if intLatencyPeaks > 1:
            vecLatencies = [vecLatencies] * intLatencyPeaks
    ...
    if len(vecLatencies) == 1:
        vecLatencies = vecLatencies[0]
